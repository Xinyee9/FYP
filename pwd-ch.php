...
function send_password_reset($get_name,$get_email,$token)
{
    ...
    $mail->Username   = "ngweewoen0415@gmail.com";              //SMTP username
    ...
    //Recipients
    $mail->setFrom("ngweewoen0415@gmail.com", $get_name);
...
if(isset($_POST['password_update']))
{
    ...
    if(!empty($token))
    {
        if(!empty($email) && !empty($new_password) && !empty($confirm_password))
        {
            ...
            $check_token = "SELECT verify_token FORM users WHERE verify_token='$token' LIMIT 1";
            ...
            if(mysqli_num_rows($check_token_run) > 0 )
            {
                ...
                {
                    $update_password = "UPDATE uesrs SET userpassword='$new_password' WHERE verify_token='$token' LIMIT 1 ";
                    $update_password_run = mysqli_query($con, $update_password);

                    if($update_password_run)
                    {
                        $new_token = md5(rand()."funda");
                        $update_to_new_token = "UPDATE uesrs SET verify_token='$new_token' WHERE verify_token='$token' LIMIT 1 ";
                    ...
                    else
                    {
                        ...
                        header("Location: pwd-ch.php?token=$token&email=$email");
                        exit(0);
                    }
...
<body>
  ...
  <form action="" method="POST" >
      ...
      <p>
        New Password:
        <input type="text" name="new_password" size="50" maxlength="55" placeholder="Enter new password" required />
      </p>
      <p>
        Confirm password:
        <input type="text" name="confirm_password" size="50" maxlength="55" placeholder="Confirm password" required />
      </p>
